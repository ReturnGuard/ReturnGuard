...
st.markdown("""
    ...
    /* Echte Farblogik für die Buttons nebeneinander (Mobile First) */
...
# --- 4. TAB: TECHNIK & FIN-VALIDIERUNG ---
with tab_tech:
    st.subheader("Fahrzeugdetails")
    t1, t2 = st.columns(2)
    
    # FIN Feld mit 17-Zeichen-Logik
    vin_raw = t1.text_input("FIN (Fahrgestellnummer)", max_chars=17, help="Eingabe wird automatisch großgeschrieben.")
    vin = re.sub(r'[^a-zA-Z0-9]', '', vin_raw).upper() # Nur Alphanumerisch + Großbuchstaben
    
    if len(vin) > 0 and len(vin) < 17:
        t1.warning(f"⚠️ FIN unvollständig: {len(vin)}/17 Zeichen")
    elif len(vin) == 17:
        t1.success("✅ FIN korrekt (17 Zeichen)")
...
# --- 5. TAB: EXPERT-CHECK (MODULAR & FARBIG) ---
with tab_check:
    ...
    for section_name, items in sections.items():
        with st.expander(f"📦 {section_name}", expanded=True):
            for item in items:
                # Die Segmented Control Buttons nebeneinander
                ...
                # Smart-Repair Kostenfeld erscheint nur bei Mangel
                if choice == "Mangel":
...
with tab_export:
    ...
    col_b.write(f"**Prüfdatum:** {datetime.date.today().strftime('%d.%m.%Y')}")

    if st.button("🏁 GUTACHTEN FINALISIEREN"):
        if len(vin) != 17:
            st.error("❌ Fehler: Die FIN muss exakt 17 Zeichen lang sein, bevor Sie finalisieren.")
        elif not kz:
            st.error("❌ Fehler: Bitte geben Sie ein Kennzeichen an.")
        else:
            st.balloons()
            st.success(f"Gutachten für {kz} erfolgreich erstellt. Gesamtschaden: {total_minderwert} €")
            # PDF Generierung erfolgt hier im nächsten Schritt
